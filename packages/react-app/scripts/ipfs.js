...
const deploy = async () => {
  ...
  console.log(`App deployed to IPFS with hash: ${cid.toString()}`)

  console.log()

  let ipnsName = ''
  if (nodeMayAllowPublish(ipfs)) {
    console.log(`Publishing /ipfs/${cid.toString()} to IPNS...`)
    ...
    console.log(`App published to IPNS with name: ${ipnsName}`)
    console.log()
  }
  ...
  console.log(`   IPFS: ${ipfsGateway}${cid.toString()}`)
  if (ipnsName) {
    console.log(`   IPNS: ${ipnsGateway}${ipnsName}`)
    console.log()
    console.log(
